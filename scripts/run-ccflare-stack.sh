...
GUARD_SHUTDOWN_GRACE_MS=${GUARD_SHUTDOWN_GRACE_MS:-75000}
...
validate_bounded_ms() {
	...
	if [[ ! "$value" =~ ^(0|[1-9][0-9]{0,5})$ ]] || ((value < min || value > max)); then
		log "invalid ${name}=${value}; expected an integer from ${min} to ${max} milliseconds"
		exit 64
	fi
}

validate_bounded_ms GUARD_SHUTDOWN_GRACE_MS "$GUARD_SHUTDOWN_GRACE_MS" 0 600000
...
env \
	...
	GUARD_TOTAL_DEADLINE_MS=120000 \
