...
A family's `managed_model`, and an individual Manual slot's `model`, can hold either a concrete logical model ID or a bare family-alias literal — the family's own name (`opus`, `sonnet`, `haiku`, or `fable`, case-insensitive). An alias means "track the latest model in this family": it is resolved at read and request time, and is never rewritten to a concrete value by that resolution. Shipping a new latest model for a family only requires the latest-model mapping bump in `models.ts` plus a deploy; it does not require a policy migration, a new preview, or an apply step against existing alias-valued assignments or slots.

A bare alias does not always resolve to the family's latest model. If the requesting client asked for a well-formed concrete model ID in that same family (for example `claude-opus-5` while the bundled latest is `claude-opus-5-5`), the alias passes that client-requested model straight through to the account instead — whether it is older than latest (an entitlement pin, or a Claude Code build too old for the newest release), the current latest, or newer than anything in the bundled catalog yet (a same-day release the catalog hasn't caught up to). The family's latest model is used only as a fallback: for a malformed or unrecognizable requested ID, when the alias belongs to a different family than the request (a cross-family fallback slot, e.g. a Fable route's Opus backup slot receiving a Fable request), or when no member account's capability supports the requested ID at all (the resolver retries once without the requested model before falling through to ordinary-stock routing). A stored concrete pin is unaffected either way and always wins over pass-through.

**Residual gap:** pass-through only ever offers the *client's own* requested model — it cannot invent a version for a stale client routed onto a cross-family fallback slot. If a Claude Code build too old for the family's latest model gets deferred onto that kind of slot, it can still receive a model its version is not entitled to use.
