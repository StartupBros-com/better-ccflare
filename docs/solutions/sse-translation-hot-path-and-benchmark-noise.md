...
## The change that looks obvious and is wrong: chunks[].join

It is tempting to replace per-delta text accumulation

```ts
block.text += text;              // then read block.text at content_block_stop
```

with the textbook "avoid quadratic concatenation" form:

```ts
block.chunks.push(text);         // then chunks.join("") at content_block_stop
```

**In Bun/JavaScriptCore this is a pessimization.** JSC represents concatenation
as a rope: `+=` is O(1) amortized and the string is flattened once, lazily, when
it is first read. The accumulate-then-read-once shape the translator already had
is therefore close to optimal, and `chunks[].push` + `join("")` only adds array
growth and a second copy.

Measured in-process, 20,000 x 200B deltas, three runs: the `chunks[].join` form
was **+58.1%, +26.3%, +33.4% slower**. It was rejected for that reason.

The corollary also holds: because `block.text` and the `fullText` local are the
same string reference, deleting the map entry *earlier* in `content_block_stop`
frees nothing. `content_block_stop` already deletes the entry at its end; adding
a second, earlier delete buys no memory and duplicates code.
