...
- The fork forwards supported client controls through its bounded Responses carrier. Caller-supplied `previous_response_id` is ignored; continuation remains owned by the verified account/model/session lane. This does not implement upstream's raw native-request validation path.
...
Use a real interactive Codex client to send a short prompt. Scripted requests must use a non-Anthropic account and force-route with `x-better-ccflare-account-id`. A working connection must complete the response stream, not merely return HTTP 200. A client-side compatibility patch cannot fix upstream account entitlement, quota, or access-denial errors.
...
Upstream reported validation with the VS Code extension's Codex 0.153.4 app-server requesting `gpt-6-astra`: namespaced `functions.exec` ran `tools.apply_patch`, the tool result was replayed, and the final assistant message completed. The same client against unmodified v3.5.78 failed with `400 Unsupported Responses request field(s): client_metadata`.
