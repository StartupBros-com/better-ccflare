...
- **R9 — Replay writer accounting:** Enable the existing AES-GCM writer only when a persistent fleet issuance count is available. Record issuance after successful encryption. Enforce the existing rotation/exhaustion thresholds. Store counts only; never store request, query, result, URL, citation, token, or envelope content.
