...
- **R8.** Construct result and citation evidence only from provider-exposed source records and URL annotations. Preserve URL, title, `page_age` as `string | null`, and cited text when present; never invent page content, source titles, queries, or citation provenance. Anthropic text citations are a mixed union: preserve unrelated citation types unchanged, reject only a mixture of proxy-owned and native-web citations in one replay lineage, and retain each original citation index so output ordering can be restored.
- **R9.** Distinguish successful zero results from hosted-tool failure. Map only fixture-proven native failures to Anthropic `web_search_tool_result_error`, whose official error codes are exactly `too_many_requests`, `invalid_tool_input`, `max_uses_exceeded`, `query_too_long`, `request_too_large`, and `unavailable`; unknown post-dispatch loss maps conservatively to unavailable/protocol failure.
...
- **R13.** Emit versioned bounded envelopes in `encrypted_content` and `encrypted_index` as `bccf2.A256GCM.<kid>.<sloc>.<nonce>.<ciphertext-and-tag>`. `<sloc>` is a 16-byte, 22-character canonical unpadded-base64url locator computed with a separately HKDF-derived HMAC-SHA256 key over the canonical common source identity: protocol/suite, tool type, trusted replay audience and conversation lineage, call ID, normalized visible-query keyed digest, result/error state, source ordinal, and the ordered source URL/title/`page_age`. It deliberately excludes cited text, citation-specific linkage and ordinal, provider/model/fidelity, issuance time, nonce, and wire key ID so one result source and its citations share a locator. Canonical associated data includes envelope kind (`source` or `citation`), the locator, and every exact binding/evidence visible digest reconstructible before decryption. Authenticated ciphertext stores provider/model provenance, fidelity level, issuance time, bounded evidence counts, and keyed visible-field digests rather than duplicating large visible strings; it contains no credentials, prompt bodies, or unexposed page content.
- **R14.** Validate envelope namespace, six-segment header, key ID, source locator, AEAD tag, schema, size, audience, conversation/call linkage, query/result state, evidence order, and visible-field digests before using replay evidence. History first parses the bounded header, maps `(kid, sloc)` to exactly one prior source, and performs at most one AES-GCM decrypt per unique token; duplicate URL/title sources remain distinct through call and source ordinal. Unknown key, bad tag, malformed schema, ambiguous locator, wrong linkage, truncation, and context mismatch return the same client-visible invalid-envelope class. Every old or unknown `bccf*` namespace is locally invalid rather than native-forwarded. Cap aggregate envelopes at 512, encrypted input at 1 MiB, and replay tokens at 4 KiB each before any provider I/O.
...
- **R17.** Historical server-tool blocks never trigger a new search. Mixed chronology among prior server results, assistant text, client function calls, client tool results, and unrelated citation-union members must be preserved, using original citation indices when proxy citations are replaced.
...
- **AE17. Replay round trip**
  - **Given:** A prior response contains valid proxy-issued result and citation envelopes, including two sources with the same URL/title, plus unrelated citation-union members.
  - **When:** a later turn is routed through a different projection-capable provider.
  - **Then:** bounded headers select each exact source by `(kid, sloc)`, each unique token incurs at most one AES-GCM decrypt, duplicate sources remain distinct, unrelated citations and original ordering survive, visible fields match, evidence chronology is preserved as untrusted prior context, and the historical search is not executed again.

- **AE18. Replay tamper, recovery, and key rotation**
  - **Given:** An explicit active key ID, retained previous keys, a protected backup restored into a clean codec process, valid cross-process tokens, plus tampered, old/unknown-namespace, unknown/revoked-key, reordered-key-file, oversized, cross-conversation, wrong-call, locator-collision, reordered-evidence, query-substitution, and visible-field-mismatch tokens.
  - **When:** replay decoding runs in a fresh codec instance.
  - **Then:** backup restoration and rotation preserve active and retained previous-key decoding regardless of file ordering; every invalid token produces the same invalid-envelope class before upstream I/O; every `bccf*` token stays in the local namespace; no plaintext or native-payload fallback exists.
...
- `web_search_tool_result` is emitted as one complete block when source evidence or a proven empty-result state is known. Source `page_age` preserves the official `string | null` contract. Result errors use only `too_many_requests`, `invalid_tool_input`, `max_uses_exceeded`, `query_too_long`, `request_too_large`, or `unavailable`.
- URL citations are attached only when offsets can be validated against the exact emitted text. `cited_text` is sliced from that output; invalid offsets are a fidelity error, not a guessed citation. Text citation arrays remain unions: unrelated citation kinds pass unchanged and retain their original indices for final ordering, while a proxy/native-web mixture rejects.
- Proxy envelopes pin the six-segment wire suite as `bccf2.A256GCM.<kid>.<sloc>.<nonce>.<ciphertext-and-tag>` using canonical unpadded base64url segments, AES-256-GCM, a 32-byte key, a fresh independent 12-byte nonce from the OS CSPRNG, and a 16-byte authentication tag. Key IDs match `[A-Za-z0-9][A-Za-z0-9_-]{0,63}`. Header/AAD and plaintext use schema-defined positional UTF-8 JSON tuples so byte encoding is deterministic across processes. Version and suite are authenticated; unknown versions/suites, padded/non-canonical encodings, unsafe key IDs, RNG failure, and downgrade attempts reject uniformly. Nonces are never derived from time, user input, or plaintext.
- `<sloc>` is the first 16 bytes of HMAC-SHA256, encoded as 22 canonical base64url characters, using a separately HKDF-derived locator key over the common source identity: protocol/suite, tool type, trusted audience and lineage, call ID, query digest, result state, source ordinal, and ordered source URL/title/nullable page age. Cited text, citation-only linkage/ordinal, provider/model/fidelity, issuance time, nonce, and wire key ID are excluded so the result source and every citation to it share one opaque locator. Envelope kind (`source` or `citation`), the locator, exact context/linkage, and visible evidence digests remain authenticated in full AAD.
- History parses only a bounded token header first, resolves `(kid, sloc)` to exactly one prior source, and then performs at most one AES-GCM decrypt for each unique token. Call/source ordinal keeps duplicate URL/title records distinct. Every old or unknown `bccf*` version is a locally invalid proxy token and is never forwarded as native Anthropic state. The abandoned `bccf1` draft never shipped, never had writer admission, and therefore has no compatibility reader; making this correction before release avoids carrying an unsafe ambiguous legacy format.
- This stateless design provides probabilistic nonce uniqueness, not an atomic cross-process guarantee: the operational AES-GCM budget is fewer than `2^32` envelopes for the same raw key material, with rotation/stop at `2^31`, based on trustworthy fleet telemetry. Fleet key identity and issuance accounting are version-independent so a wire-version change cannot reset the nonce budget. If that fleet count is unavailable, writer admission remains off.
...
| Hosted-tool result error | Fixture-proven native mapping to one of the six official codes (`too_many_requests`, `invalid_tool_input`, `max_uses_exceeded`, `query_too_long`, `request_too_large`, `unavailable`) | In-band `web_search_tool_result_error` | No duplicate search |
...
- **KTD8. Pin an audience-bound replay wire suite with explicit source location and key activation.** One-shot-only success would break the next Claude turn; bearer tokens without conversation/call/query binding are transferable; a token that cannot identify one exact duplicate source before decryption cannot meet the single-decrypt stateless contract; algorithm agility without an authenticated suite invites downgrade; and order-selected or environment-carried keys make rotation unsafe. `bccf2` pins AES-256-GCM, a separately derived deterministic source locator, source/citation envelope kind, canonical tuple encoding, independent 96-bit OS-CSPRNG nonces, 128-bit tags, unpadded base64url, protected key files, explicit active key IDs, retained decryptors, keyed visible-field digests, version-independent key-material issuance accounting, an honestly operational—not atomically enforced—fleet budget, and decoder-floor rollout.
...
- **Approach:** Write failing codec and hostile-projection tests first. Load a protected key file through central configuration, require an explicit active key ID and unique 256-bit key material, reject duplicate JSON members, unsafe file permissions/reordering ambiguity, and an inode that changes during the bounded read, and inject the initialized codec through `ProxyContext` with writer admission disabled in U2. Pin and validate the final regular inode before opening it for payload reads. Pin the six-segment `bccf2.A256GCM.<kid>.<sloc>.<nonce>.<ciphertext-and-tag>` format to AES-256-GCM with an independent 12-byte OS-CSPRNG nonce, 16-byte tag, canonical unpadded base64url, and schema-defined positional UTF-8 JSON tuples; authenticate the version/suite and fail if entropy is unavailable. Use a separately HKDF-derived HMAC-SHA256 key to calculate the 16-byte locator from common source identity, excluding citation-specific/provider/time/nonce/wire-key-ID data so one result and its citations share it. Build full AAD from pre-decryption reconstructible header/context values, including envelope kind, locator, protocol/tool, trusted ingress audience, session/affinity lineage, call ID, visible-query keyed digest, result/error state, evidence order/linkage, and visible digests. Keep provider/model provenance, fidelity, issuance time, bounded evidence counts, and keyed visible-field digests inside authenticated ciphertext. Validate all aggregate/per-token limits, pre-index exact prior sources by `(kid, sloc)`, and perform at most one AES-GCM decrypt per unique token before provider input construction, then render escaped length-framed structured evidence with canonical credential-free HTTP(S) URLs. Preserve unrelated citation-union members and their original indices, but reject a proxy/native-web mixture. Detect non-proxy Anthropic opaque payloads separately and preserve them byte-for-byte for native routes, while reserving every `bccf*` namespace for uniform local validation failure. Never import the database payload-encryption helper because its optional plaintext behavior and package ownership do not meet this wire contract.
- **Test scenarios:**
  - Fixed cross-process vectors pin exact six-segment `bccf2.A256GCM` encoding, the 16-byte/22-character deterministic locator, source/citation envelope kind, 12-byte nonce, 16-byte tag, canonical tuple bytes, explicit-active-key encode/decode, retained-previous-key decode, key-array reordering, process restart, and old/new-process cross-decryption.
  ...
  - Unknown/old `bccf*` version or suite, non-canonical or padded base64url, unsafe key ID, invalid locator, injected RNG failure, and algorithm downgrade fail closed without native forwarding. Tests prove one OS-CSPRNG request per issuance and no caller/time/plaintext nonce input; they do not claim detection of an unpredictable cross-process random collision.
  - Injected fleet-counter tests exercise the `2^31` rotate/stop signal and missing-telemetry admission gate; key-material identity remains stable across protocol versions and documentation explicitly treats the `<2^32` budget as operational rather than an atomic codec invariant.
  - Tamper, unknown/revoked/lost key, duplicate ID, malformed key file, unsafe permissions, missing active ID, oversized token/aggregate, source-count overflow, duplicate URL/title sources, ambiguous or mismatched `(kid, sloc)`, cross-audience/conversation/call/version/query/state/order mismatch, wrong linkage, and truncation all produce one invalid-envelope client class. Header pre-indexing proves at most one AES-GCM decrypt per unique token.
  ...
  - `page_age` preserves both strings and explicit null; the six official result-error codes are accepted and stale/nonexistent values reject.
  - Mixed prior search, answer text, client function, tool result, proxy web citation, and unrelated citation-union chronology survives projection using original citation indices; only proxy plus native-web citation mixtures reject.
...
- exact six-segment `bccf2.A256GCM` wire vectors, deterministic source-locator and source/citation-kind behavior, suite/version downgrade behavior, nonce/tag/encoding rules, active/retained key behavior, version-independent key-material identity, and probabilistic-nonce/operational-budget rotation evidence;
...
- Verify trustworthy aggregate issuance telemetry across all writers using a version-independent hashed identity of the raw AES key material, and rotate/stop at `2^31`; if the fleet count is unavailable, keep admission off. The plan makes no atomic stateless guarantee: the hard operational stop condition remains below the documented `2^32` budget.
...
- Proxy-owned `bccf2.A256GCM.<kid>.<sloc>.<nonce>.<ciphertext-and-tag>` envelopes pin six-segment key/locator/nonce/tag/base64url/canonical-tuple bytes, source/citation kind, and exact visible evidence bindings; survive process restart, key reordering/rotation, compatible rolling deploy, and decoder-floor rollback; and resolve duplicate URL/title sources by `(kid, sloc)` with at most one AES-GCM decrypt per unique token. Independent OS-CSPRNG nonces plus trustworthy version-independent key-material telemetry support the explicitly probabilistic nonce model and prove rotation before the operational budget. Every old/unknown `bccf*` token rejects uniformly, and valid envelopes support the next Claude turn without DB state or re-execution.
