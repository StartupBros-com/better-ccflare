...
- **Test scenarios:**
  ...
  - Covers AE2. Source `header_agent`: the marker is present.
  - Covers AE2. Source `session_header` plus the parent-agent header; plus the agent-id header; plus a billing header containing the subagent field set to true: the marker is present in each case.
...
2. **Prerequisites, checked at deploy and again on day 1 and day 7:** the service's active environment still carries the Codex trace directory (read the local drop-in or the unit's shown environment); the demotion warn line reaches only the logger's file sink under the temp directory, not the journal.
3. **Day 1 and day 7 reads** (trace files newer than the deploy; use `rg -N` with snake_case keys, since the trace lines are not parseable by `jq`):
