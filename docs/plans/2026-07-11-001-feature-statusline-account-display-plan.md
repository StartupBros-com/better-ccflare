...
| Product Contract preservation | Requirements text unchanged. The three Outstanding Questions (all deferred-to-planning) are resolved into KTD-3, KTD-6, and KTD-7 and the section removed. Planning adds one behavior clarification within the R2/R5 spirit: pool exhaustion clears the session's map entry (KTD-5). |
...
- Assumes per-account usage-toward-limit and rate-limit/paused state are already available to the API layer. Verified: `usageUtilization`, `usageWindow`, `paused`, `rateLimitStatus`, and `rateLimitedUntil` are already assembled for `GET /api/accounts` in `packages/http-api/src/handlers/accounts.ts` via `getRepresentativeUtilization` and `usageCache` from `packages/providers/src/usage-fetcher.ts`.
...
- KTD-5. **Pool exhaustion clears the session's entry.** When `handleProxy` fails a request without any account serving it, delete the map entry for that request's session id. Two structurally different exit paths in `packages/proxy/src/proxy.ts` need the `clearSession` call: the no-accounts-selected branch that returns the pool-exhausted 503 inline (the `accounts.length === 0` check), and the post-loop all-candidates-failed branch, which instead throws `ServiceUnavailableError` (two throw sites: the needs-reauth variant and the final generic throw after the retry loop exhausts every candidate). Call `clearSession` immediately before each of these exits in `proxy.ts`, not in the generic error catch in `apps/server/src/server.ts`, since that catch handles all proxy errors, not just pool exhaustion. `forwardToClient` never runs on these paths, so without the clears the badge would keep showing the last healthy account while the pool is down, undercutting the health-visibility motivation. This is a planning clarification within R2/R5: after a failed request no account "actually served", so the association degrades to unknown.
...
- **Test scenarios:**
  ...
  - Integration: driving `handleProxy` into the no-accounts (503) path and the all-candidates-failed path with a session header present clears a previously recorded entry (verifies the KTD-5 wiring, not just the observer module).
