...
| `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES` | Bun proxy process-local weighted body-admission budget. Valid values are 256 MiB–1 GiB; it is independent of the front guard's encoded-buffer budget | `268435456` (256 MiB) | `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES=536870912` |
...
The Bun proxy has a separate, process-local admission controller for authenticated body-bearing proxy routes. `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES` defaults to **256 MiB** (`268435456` bytes) and accepts only 256 MiB–1 GiB. `CCFLARE_MAX_BODY_ADMISSION_QUEUE` defaults to **500** and accepts 0–5000. These settings neither share a reservation nor form a lock with the Node guard's 256 MiB encoded-buffer budget: each process protects its own work independently.

Before a proxy handler reads a request body, Bun reserves a fixed conservative weight of **8×** a canonical unencoded/`identity` `Content-Length`, up to the 32 MiB request limit. Zero reserves zero. Missing, chunked, malformed, over-limit, compressed, and unknown-encoding metadata instead reserve the complete Bun budget. The multiplier is an admission weight, not a measurement or cap for heap/RSS. Exact request-size enforcement remains with the downstream body readers.
