...
|-------|------|---------|-------------|
| `lb_strategy` | string | `"session"` | Load balancing strategy. Supported values are `"session"` (default), `"session-affinity"`, `"session-drain-soonest"` (an opt-in affinity-preserving drain policy), and `"least-used"`. Prefer a session-based strategy for OAuth accounts; per-request spreading can trigger provider anti-abuse systems |
...
| `session-drain-soonest` | Opt-in session-affinity variant. Structural routing classes remain authoritative; within one authorized class, eligible auto-fallback candidates are considered first, then candidates rank by earliest known future all-model weekly reset, priority, and utilization. Unknown or stale resets fail open; explicit retain-owner and route-circuit decisions remain authoritative | OAuth pools where weekly capacity should be consumed before it expires while preserving per-client/lane stickiness |
...
| `CCFLARE_DISABLE_COMBO_SESSION_FALLBACK` | **Legacy.** The setting now lives in Settings → Advanced → Combo Session Fallback and is read from the config file only. This variable is adopted once at boot when the config field is absent, then ignored. It inverts on adoption because the setting expresses whether fallthrough is *allowed*. Blocking fallthrough is the default for new installs | adopted once, then ignored | `CCFLARE_DISABLE_COMBO_SESSION_FALLBACK=true` |
...
| `GUARD_MAX_REQUEST_BODY_BYTES` | Maximum request wire body retained by the front guard before it returns a local 413. Limited requests acquire guard admission before body buffering; valid values are 1 KiB–32 MiB | `33554432` (32 MiB) | `GUARD_MAX_REQUEST_BODY_BYTES=16777216` (16 MiB) |
| `GUARD_MAX_BUFFERED_REQUEST_BODY_BYTES` | Aggregate weighted reservation budget for guarded request bodies. Must be at least twice `GUARD_MAX_REQUEST_BODY_BYTES` and no more than 256 MiB | `268435456` (256 MiB) | `GUARD_MAX_BUFFERED_REQUEST_BODY_BYTES=33554432` (32 MiB; requires request limit ≤16 MiB) |
| `GUARD_MAX_BODY_READERS` | Maximum concurrent bounded request-body readers. This independent cardinality pool is separate from the aggregate reservation budget and upstream permits, so operators may tune it without weakening byte bounds | `8` (or `2 * GUARD_MAX_ACTIVE`, whichever is larger; hard max 256) | `GUARD_MAX_BODY_READERS=16` |
| `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES` | Bun proxy process-local weighted body-admission budget. Valid values are 256 MiB–1 GiB; it is independent of the front guard's encoded-buffer budget. The managed production pin is 1 GiB (`1073741824`) | `268435456` (256 MiB) | `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES=536870912` |
| `CCFLARE_MAX_BODY_ADMISSION_QUEUE` | Maximum FIFO waiters for Bun proxy body admission. Valid values are 0–5000; `0` rejects immediately when the weighted budget is full | `500` | `CCFLARE_MAX_BODY_ADMISSION_QUEUE=100` |
...
| `CCFLARE_CODEX_AFFINITY_HEADERS` | Enabled by default: send the ChatGPT subscription backend's prompt-cache affinity headers on every converted Codex request and WebSocket handshake — `session-id` and `thread-id` set to the request's `prompt_cache_key`, plus `x-codex-routing-hint: model=<physical model>`. The backend derives cache affinity from the `session-id` header, exactly as the official Codex CLI sends it (openai/codex `core/src/client.rs`); with only the body key, byte-identical continuations on one account measured ~45% zero-cache in Sept 2026 while the CLI stayed above 93%. A native `/v1/responses` client's own `session-id`/`thread-id` are preserved; legacy `/v1/messages` clients cannot supply them. The routing hint always follows the proxy's resolved physical model. Off the exact subscription endpoint the headers are left untouched. Set to `0` to restore pass-through behavior | `1` | `CCFLARE_CODEX_AFFINITY_HEADERS=0` |
...
| `CCFLARE_CODEX_CACHE_KEY_SESSION_PERCENT` | Compatibility canary for comparing conversation and session cache-key modes on eligible OpenAI endpoints. Session-wide keys concentrated unrelated agent fan-out and are retired as a parity mechanism; keep this at `0` outside targeted diagnostics. Only unsigned base-10 integers are accepted, values above `100` clamp, and an explicit `CCFLARE_CODEX_CACHE_KEY_MODE=session` still takes precedence | `0` | `CCFLARE_CODEX_CACHE_KEY_SESSION_PERCENT=10` |
...
| `CCFLARE_CODEX_CACHE_KEY_PREFIX_SHARD_PERCENT` | Compatibility canary for sharing a SHA-256-only `prompt_cache_key` by physical model, converted instructions/tools, and one of eight deterministic shards. Warmed natural traffic reached 86.16% reuse at no more than 13 requests/minute/key, so this is retired as a parity mechanism and stays at `0` unless a new backend contract justifies re-testing. It applies only to the exact ChatGPT subscription endpoint | `0` | `CCFLARE_CODEX_CACHE_KEY_PREFIX_SHARD_PERCENT=10` |
| `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS` | Compatibility control for an extra hold after clean Codex response EOF. A 6-second natural-traffic treatment reached 85.13% reuse versus 87.36% matched control, so terminal settle is retired as a parity mechanism and stays at `0`. The normal `CCFLARE_CACHE_PACING_MS` follower cap remains authoritative; values above `10000` clamp to `10000` | `0` | `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS=6000` |
| `CCFLARE_CODEX_GPT56_EXPLICIT_CACHE_BREAKPOINT_PERCENT` | Default-off treatment that marks a historical stable GPT-5.6 input block. The ChatGPT subscription endpoint rejected 26/26 Sol/Terra treatments, so keep this at `0`; account/model/endpoint-scoped suppression and one pre-content marker-free retry remain as compatibility protection. Re-test only after the private endpoint accepts the documented marker on naturally initiated traffic | `0` | `CCFLARE_CODEX_GPT56_EXPLICIT_CACHE_BREAKPOINT_PERCENT=10` |
| `CCFLARE_CODEX_MESSAGES_CONTINUATION` | Opt-in: let the Codex provider retain server-owned Responses continuation (`previous_response_id`) while serving Anthropic `/v1/messages` clients, instead of replaying the full translated conversation history every turn. Requires an authenticated API key, a selected account, and a nonempty prompt cache key; state is partitioned by account, model, authenticated caller, session, and protocol, so a Messages-lane checkpoint can never leak into native `/v1/responses` traffic. Ambiguous, failed, expired, or cancelled turns never create or reuse a checkpoint, a rejected `previous_response_id` triggers a one-time full-history repair rather than looping, and the switch never changes model selection, fallback policy, or `store:false`. See [README](../README.md#-codex-messages-lane-continuation-opt-in-off-by-default) for details | unset (off) | `CCFLARE_CODEX_MESSAGES_CONTINUATION=1` |
...
### Codex cache pacing settle compatibility

The proxy still binds terminal behavior after successful routing using the actual winning provider, preserves first-chunk release for every non-Codex winner, releases immediately on cancellation/error, and bounds the hold by the existing liveness failsafe. This surface remains for targeted diagnosis, not staged promotion. Roll back with `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS=0` and restart.

### Codex cache parity ownership

`GET /api/insights/cache-parity` is the read-only operator surface. Its rolling seven-day verdict is authoritative; the 24-hour window is advisory. It reports additive durable usage as logical-final scope and keeps schema-20 physical-attempt pacing/cache analysis separate. A nonzero global keepalive TTL makes the durable verdict `insufficient_evidence` until synthetic request origin is persisted.

The response's `driftReport` field diffs the live policy snapshot against the declared defaults in `packages/http-api/src/services/cache-parity-expectations.ts` (which mirrors the KEEP/DEFER/RETIRE table below) and reports whether the measured floor — 75% weighted cache-read, 12% zero-hit ceiling for Codex follow-ups — is currently held (retuned in #233/#238 for real drift margin against the 79.1% / 9.1% achieved level). A lever that differs from its declared default shows up as a `deviations` entry; `CCFLARE_CODEX_TURN_STATE_PERCENT=50` ships pre-acknowledged citing #199, so it reports as a known, owned deviation rather than an anomaly. Any other nonzero lever reports unacknowledged until it is either retired back to its default or added to that file's acknowledgement map with an owning issue.

| Lever | State | Permanent contract or re-entry condition |
|---|---|---|
| Conversation `prompt_cache_key`, exact-account/session affinity, ordinary 60-second first-chunk pacing | KEEP | Preserve on the Claude Code → better-ccflare → ChatGPT subscription path |
| `session-id`/`thread-id` affinity headers carrying the conversation `prompt_cache_key`, plus `x-codex-routing-hint` | KEEP | The subscription backend derives cache affinity from the `session-id` header (official Codex CLI contract); the body key alone stopped holding a warm machine in Sept 2026. Schema-21 traces record `affinity_session_identity` / `affinity_routing_hint` per request |
| Bounded orchestration continuity | KEEP | Apply only after identity or exact tool-lineage admission rejects siblings and unrelated instructions |
| Source-correct metrics and physical model/account/turn/gap/context reporting | KEEP | Durable additive usage uses `read / (uncached + read + write)`; cache-inclusive traces use `read / total_input` |
| Pacing role, wait, and release receipts | KEEP | Schema-20 traces classify leader, follower/leader release, follower/cap release, or unknown without changing requests |
| Explicit GPT-5.6 stable boundary | DEFER | Re-open only when the private subscription endpoint accepts the documented marker on naturally initiated Sol and Terra traffic |
| Backend publication, replica consistency, and cross-thread cache lineage | DEFER | Requires an OpenAI backend change or documented affinity/lineage contract |
| Session keys, prefix sharding, active WebSocket locality, turn-state replay as parity, terminal settle | RETIRE | Compatibility surfaces remain default-off; re-open only with a new causal backend contract, not another percentage or shard count |
| Retry after a successful zero-cache response; Codex CLI compaction/fork/side work | RETIRE | Successful tool-bearing requests are not replay-safe, and Codex CLI is outside this production path |
...
| `CCFLARE_CODEX_TURN_STATE_PERCENT` | Default-off compatibility percentage for the official Codex HTTP same-turn sticky-routing contract. Measured replay was null as a parity lever; use only for narrow protocol diagnosis. Account/model/cohort gates remain mandatory, malformed values become `0`, and values above `100` clamp | `0` | `CCFLARE_CODEX_TURN_STATE_PERCENT=100` |
...
| `CCFLARE_CODEX_TURN_STATE_COHORT_SENTINEL` (value `*` inside `CCFLARE_CODEX_TURN_STATE_COHORT_IDS`) | A cohort is scoped to one conversation, so it does not exist until that conversation is already running and it dies with it — naming one in advance enrols nothing, which makes the exact-cohort gate unusable for a forward-looking rollout. Setting the allowlist to `*` opts in to letting the deterministic percentage select cohorts instead. Deliberately a distinct token rather than "empty means all": an **empty allowlist still means no treatment**, which is what makes shipping dark safe. Account and model allowlists, the percentage, and observe-only all still gate ahead of it | empty | `CCFLARE_CODEX_TURN_STATE_COHORT_IDS=*` |
...
### Codex HTTP same-turn sticky-routing compatibility
...
| Converted input appended after the matching tool output, such as the continue-now nudge added after a final Skill result | No replay; the request that reaches Codex is no longer an exact same-turn continuation even though its lineage matches. The tail of the converted body is checked, so any future append is covered without naming it |
...
| Tool calls announced to the client that do not pair exactly with a completed call — one of several parallel calls left open at the terminal, a completion without a matching start, a completion whose call ID differs from the one its start announced, a reused or absent output index | Never capture. A lineage covering only the completed calls, or recording the announced ID for a different completed call, would look exact while describing a different turn than upstream produced |
| Entry-cap eviction removing the generation, pending turn, or attempt a replay decision just rested on | No replay. The cap is a hard bound and can drop in-flight state, so the token is withheld rather than sent with nothing left to track it to a terminal |
| Transport failure after the request reached the wire, such as a socket, TLS, timeout, or abort error with no response | Release the attempt so it stops holding its request's lease, but keep and refresh the pending turn: the official contract's answer to an unknown send is to replay the same token on a compatible retry. Refreshing matters when the attempt ran past the idle TTL — once the attempt is removed, the next sweep would otherwise expire the retained token immediately using its pre-attempt timestamp. No `attempt_aborted` record is written, because the request really was sent |

Treatment retains the raw token only in bounded process memory. Observe mode retains no token or lineage; control shadow state retains only token presence and domain-separated call-ID fingerprints. Schema-18 Codex traces store bounded arm/action/terminal categories, an opaque cohort, and keyed token HMACs for request/response matching. A request record is written while the body is transformed, which is before route claiming, so a candidate that is then abandoned — a duplicate route claim, a superseded fallback, or a commitment budget expiring before dispatch — is annulled by an `attempt_aborted` record carrying only the request and attempt IDs its request record already held. An abandoned candidate also rolls back the account and model it stamped as the request's last route, so the next attempt that does reach the wire is not attributed as a fallback from a route that never sent. Hosted-search requests are reported without registering an attempt at all, because hosted responses finalize through their own processor and would otherwise leave an attempt that no terminal can resolve. Analysis drops both records, so requests-per-key pressure, fallback totals, and final-attempt attribution count only attempts that reached the wire. Those turn-state fields never carry the raw token, raw call IDs, lineage sets, session/account IDs, or full cache keys. That guarantee covers the turn-state telemetry only. The separate opt-in `CCFLARE_CODEX_TRACE_FULL=1` mode embeds complete Anthropic and Codex request bodies in the same records, and those bodies do contain raw tool-call IDs, `metadata.user_id` session data, and the full prompt cache key. Full-trace mode records bodies rather than headers, so it never gains the raw turn-state token, but treat its output as sensitive and leave it off for rollout review. The cache-experiment analyzer reports only aggregate treatment/control cache, error, retry, latency, context/gap, HMAC-match, and prompt-key-concentration metrics.

If this compatibility path needs diagnosis, use only naturally initiated authorized Codex traffic and obtain fresh operator approval before observe-only or treatment. Keep WebSocket treatment off, scope one exact account/model/cohort, and read key pressure from the report-wide `prompt_key_concentration` summary rather than partitioned rows. Do not use a favorable result to claim or promote cache parity. Roll back by setting `CCFLARE_CODEX_TURN_STATE_PERCENT=0`, clearing `CCFLARE_CODEX_TURN_STATE_COHORT_IDS`, and restarting; no migration or data cleanup is required.

| `CCFLARE_CODEX_WS_PERCENT` | Default-off transport compatibility canary for the official ChatGPT-subscription Responses WebSocket. Natural traffic measured 75.43% cache reuse, worse than HTTP, so active WebSocket locality is retired as a parity mechanism. Assignment remains stable per account/cache key and requires account/model allowlists for targeted non-parity diagnostics | `0` | `CCFLARE_CODEX_WS_PERCENT=10` |
...
| `CCFLARE_CODEX_IMPLICIT_ROUTE` | Admits physical Codex model IDs on `/v1/responses` through a request-time Codex capability route, preserving the requested wire model. Enabled by default; only the literal value `0` disables it. Explicit operator profiles take precedence; model discovery and session binding behavior are unchanged. See [Implicit Codex routes](./routing-architecture.md#implicit-codex-routes) | `1` (on) | `CCFLARE_CODEX_IMPLICIT_ROUTE=0` |
...
`GUARD_MAX_REQUEST_BODY_BYTES` bounds the request **wire** body retained by the local front guard. The managed deployment sets **32 MiB** (`33,554,432` bytes); valid values are 1 KiB through 32 MiB. `GUARD_MAX_BUFFERED_REQUEST_BODY_BYTES` is the separate aggregate reservation budget, managed at **256 MiB** (`268,435,456` bytes). It may not exceed 256 MiB and must remain at least twice the per-request limit. At the defaults, the 256 MiB budget admits four worst-case 32 MiB bodies because each reserves 64 MiB. Invalid, missing, or incompatible settings are rejected before a managed restart. A declared `Content-Length` above the wire limit is rejected before buffering. Chunked or misleading uploads are counted as bytes arrive and stopped at that same limit; the guard drains the remainder so keep-alive parsing stays synchronized. The client receives `413` with error type `guard_request_body_too_large`.

For limited inference paths (`/v1/messages` except `count_tokens`, and `/v1/complete`), the guard first reserves fair admission **before** attaching body listeners or buffering bytes. A valid `Content-Length` reserves twice its declared size (weighted admission); missing, chunked, invalid, or misleading lengths reserve twice the full per-request limit. The aggregate reservation prevents the retained-body total from exceeding its budget. `GUARD_MAX_BODY_READERS` is an independent reader-cardinality limit, not a managed pin: it and the byte budget can be tuned separately. A completed body keeps its reader slot until it acquires an upstream permit. Waiting readers are FIFO, so a large reservation at the head can intentionally block smaller later uploads until enough aggregate budget is released; this preserves fairness rather than bypassing the queue. Queue-full, shutdown, and abort decisions therefore happen before an unadmitted request can consume an aggregate body buffer. Other paths still receive the wire-size bound but do not consume a limited-path admission slot.

The OpenAI Responses endpoint applies the same 32 MiB bound to both the encoded request body and its decoded `gzip`, `deflate`, or `zstd` representation; compressed expansion cannot bypass the wire limit. Rejected, oversized, invalid-target, and shutdown uploads are drained only for the configured drain window (`GUARD_REQUEST_DRAIN_TIMEOUT_MS`). If the client does not finish, the guard destroys that request socket and emits `guard_request_drain_timeout`; this prevents a stalled peer from pinning a keep-alive connection or file descriptor indefinitely.

Both limits are restart-scoped in the guard process. Lower `GUARD_MAX_REQUEST_BODY_BYTES` and `GUARD_MAX_BUFFERED_REQUEST_BODY_BYTES` as a compatible pair: keep the aggregate at least twice the lowered request limit. After a restart, observe naturally arriving authorized traffic rather than generating synthetic traffic, and confirm root and `runtime.limits` `maxRequestBodyBytes`/`maxBufferedRequestBodyBytes` in `/_guard/health`. Use root `bodyReaders.reservationLimitBytes`, `reservedBytes`, and `reservedBytesPeak` to compare the configured budget, current reservations, and high-water mark; correlate 413s with `guard_request_body_too_large` and `counters.oversizedRequestBodies`. No request content is retained in these diagnostics.

### Bun Proxy Body Admission

The Bun proxy has a separate, process-local admission controller for authenticated body-bearing proxy routes. `CCFLARE_MAX_BUFFERED_REQUEST_BODY_BYTES` defaults to **256 MiB** (`268435456` bytes) and accepts only 256 MiB–1 GiB; the managed production deployment pins it to **1 GiB** (`1073741824` bytes). `CCFLARE_MAX_BODY_ADMISSION_QUEUE` defaults to **500** and accepts 0–5000. These settings neither share a reservation nor form a lock with the Node guard's 256 MiB encoded-buffer budget: each process protects its own work independently.

Before a proxy handler reads a request body, Bun reserves a fixed conservative weight of **8×** a canonical unencoded/`identity` `Content-Length`, up to the 32 MiB request limit. Zero reserves zero. Missing, chunked, malformed, over-limit, compressed, and unknown-encoding metadata — and any caller that must hold the full worst case up front, such as the OpenAI Responses routes while read, decompress, parse, and translate are in flight — instead reserve the worst case: `8 × 32 MiB = 256 MiB`, capped at the configured budget rather than claiming its entirety. At the managed **1 GiB** pin, that worst-case reservation admits four concurrent worst-case requests before a fifth queues; at 24-way concurrency the same 1 GiB budget admits roughly **5.3 MiB** bodies concurrently. The multiplier bounds weighted admitted work, not heap or RSS; it is an admission weight, not a direct measurement. Exact request-size enforcement remains with the downstream body readers.

Admission is FIFO: a request at the head may intentionally block smaller later bodies until it fits. A full queue returns a local `503` with `Retry-After: 1` before proxy/provider work starts; aborted queued requests are removed. Restarting clears queued and active process-local state. `GET /health` exposes only aggregate `runtime.bodyAdmission` configuration, current/peak reservations, active/queued counts, and lifecycle counters—never paths, accounts, session/request identifiers, or payloads.
...
|----------|---------|---------|
| `ALERT_CACHE_HEALTH_ENABLED` | Background evaluation on/off; accepts `1`/`true`/`0`/`false` | `true` |
| `ALERT_CACHE_HEALTH_THRESHOLD_PERCENT` | Warning threshold for recorded input-token reuse; integer clamped to `[1, 100]` | `90` |
| `ALERT_CACHE_HEALTH_DURATION_MINUTES` | Consecutive qualified evidence duration; rounded up to a whole ten-minute bucket and clamped to `[10, 1440]` | `30` |
| `ALERT_CACHE_HEALTH_MIN_REQUESTS` | Minimum successful measured requests per reuse bucket, or eligible completions per telemetry bucket; integer clamped to `[1, 1000000]` | `10` |
| `ALERT_CACHE_HEALTH_MIN_INPUT_TOKENS` | Minimum total input tokens per reuse bucket; integer clamped to `[1, 1000000000]` | `100000` |
| `ALERT_CACHE_HEALTH_REMINDER_MINUTES` | Elapsed time between reminders while fresh bad traffic continues; integer clamped to `[1, 43200]` | `360` |

The corresponding file keys are lowercase (for example, `alert_cache_health_duration_minutes`). Environment values take precedence. Invalid or nonfinite numeric settings fall back to the shared policy default; finite numbers are clamped and fractional values are rounded down, except duration, which rounds up to a whole bucket. The `/api/insights/alerts/config` payload exposes optional `cacheHealthEnabled`, `cacheHealthThresholdPercent`, `cacheHealthDurationMinutes`, `cacheHealthMinRequests`, `cacheHealthMinInputTokens`, and `cacheHealthReminderMinutes`. Older clients and partial updates preserve omitted cache settings without saving environment overrides into those fields.

Every five minutes the service examines up to three completed ten-minute buckets, with a two-minute settlement delay. Buckets are processed chronologically once per account generation/provider/physical model and per provider/physical model. The physical model prefers recorded response usage over routing fallbacks. State and alert history are committed together; only the winning evaluator publishes. Persisted streaks support durations longer than the thirty-minute catchup bound. Restarting does not replay older traffic, and missing buckets break incomplete streaks without recovering an active incident. Policy changes reset incompatible incomplete streaks. No cache scan is performed on an inference request.

Recorded reuse is the unrounded, token-weighted ratio `cache_read / (uncached_input + cache_read + cache_creation)`, expressed as a percent. It is not an average of request percentages or the fraction of requests with any cache hit. A default reuse bucket needs at least ten measured successful requests, 100,000 total input tokens, and 90% usable stored-usage coverage. Coverage is measured requests divided by eligible successful completions; unavailable and invalid stored usage reduce coverage. Failed, cancelled, truncated, internal keepalive, and known zero-input requests are excluded. NULL values remain unavailable except for protocol-specific absent cache-write counters that normalize to zero. Historical rows without a recorded account generation are unclassified rather than attributed to a potentially recreated account.

Three qualifying buckets below 90% open a warning. A scope with two earlier qualifying healthy buckets within 24 hours can instead open or escalate to critical after one qualifying bucket below `min(50, warning threshold)`. Reuse recovers after two qualifying buckets at `min(100, warning threshold + 2)` or better. Telemetry has an independent incident: after a prior reporting baseline of at least 95% coverage, three buckets below 80% stored coverage open a gap alert, each requiring at least ten eligible completions but no measured-token floor. Two buckets with at least ten eligible completions and at least 90% coverage recover that gap; restored records can still show poor reuse. No traffic is never recovery. An incident has one opening, at most one escalation, fresh-traffic reminders no sooner than six elapsed hours by default, and one recovery. Acknowledging an alert is not recovery.

Native Anthropic, Codex, and official xAI cache routes are eligible without requiring a previous hit. Other routes enroll after recorded positive reuse; that enrollment carries into subsequent all-zero buckets. Unsupported zero-only routes stay unclassified. Providers aggregate eligible account buckets before applying sample floors, so several small accounts can qualify together. A redundant single-account provider opening is suppressed when that account is already covered. Suppression does not leave an invisible active provider incident; new multi-account problems can still open, and an earlier real provider opening retains its recovery notification.
...
better-ccflare maintains a cache of the Anthropic model catalog (id, display name, creation date) used to populate model dropdowns in the dashboard (agent preferences, default agent model). It's exposed read-only via `GET /api/models` and force-refreshable via `POST /api/models/refresh` — see [api-http.md](api-http.md#model-catalog) for the endpoint reference. This is the unscoped Anthropic catalog only; per-account live discovery for Codex and OpenAI-compatible accounts (`?provider=&accountId=` and the unsaved-credential preview endpoint) is a separate mechanism — see [Model Discovery](providers.md#model-discovery).
...
- **Account selection keeps only accounts that can serve the requested model as written.** Codex accounts require their own catalog to list the model; an unprimed catalog does not establish support. Other providers retain their namespace check, since a Claude model id can only travel untranslated to a provider that speaks Claude ids. A request with no eligible account gets a `503` with `error.code: force_account_model_no_account` instead of a substitute — this also applies to a request pinned with `x-better-ccflare-account-id`.
