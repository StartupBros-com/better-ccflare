...
| Transport failure after the request reached the wire, such as a socket, TLS, timeout, or abort error with no response | Release the attempt so it stops holding its request's lease, but keep the pending turn: the official contract's answer to an unknown send is to replay the same token on a compatible retry. No `attempt_aborted` record is written, because the request really was sent |
