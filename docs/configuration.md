...
| `CCFLARE_CODEX_CACHE_KEY_PREFIX_SHARD_PERCENT` | Default-off deterministic conversation canary for sharing a SHA-256-only `prompt_cache_key` among requests with the same physical model, converted instructions, converted tools, and one of eight deterministic shards. Only unsigned base-10 integers are accepted; malformed values become `0`, and valid values above `100` clamp to `100`. It applies only to the exact ChatGPT Codex subscription endpoint, after `CCFLARE_CODEX_CACHE_KEY_MODE=session`, `CCFLARE_CODEX_CACHE_KEY_SESSION_PERCENT`, and the empty-input session fallback have selected conversation mode; custom endpoints and `api.openai.com` retain existing keys | `0` | `CCFLARE_CODEX_CACHE_KEY_PREFIX_SHARD_PERCENT=10` |
| `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS` | Default-off extra hold after a clean Codex response EOF for cache propagation. Only unsigned base-10 integers are accepted; malformed values (including whitespace, signs, decimals, and units) become `0`, and values above `10000` clamp to `10000`. Applies only when the successful serving account's exact provider is `codex`; Anthropic, all other providers, unknown/retained terminals, and Codex failovers that win on another provider release at the first chunk. The normal `CCFLARE_CACHE_PACING_MS` follower cap remains authoritative. | `0` | `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS=6000` |

### Codex cache pacing settle rollout

Set `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS=6000` only after the ordinary pacing cap is enabled. The proxy binds terminal behavior after successful routing using the actual winning account provider, so a Codex-first request that fails over to Anthropic retains first-chunk release. The hold ends at clean body EOF plus the configured delay and can never outlive the existing two-times pacing-cap liveness bound. Roll back immediately with `CCFLARE_CODEX_CACHE_PACING_SETTLE_MS=0` and restart the service.
