...
The opt-in Codex JSONL trace records a categorical `cache_key_continuity_basis` and only a bounded canonical-identity prefix. The analyzer reports continuity rows by basis, safe model family, observed turn/gap band, cache-read and cache-write measurements, key rotations, effective-key concentration, joined responses, and unknown measurements. It never adds prompts, instructions, raw call IDs, raw session UUIDs, credentials, or full cache keys to this provenance output. Existing legacy trace output remains unchanged unless the provenance section is explicitly inspected.
