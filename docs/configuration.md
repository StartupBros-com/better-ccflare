...
| `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS` | Total attempts including the original request. Integer, clamped to 1–10; invalid or non-finite values fall back to the default | `2` | `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS=3` |
...
| `CCFLARE_SERVER_ERROR_RETRY_ENABLED` | Opt-in, disabled by default in this fork. Retry a transient upstream server error (HTTP 500/502/503/504, any provider) once in place, then bench the account briefly and fail over to the next one. Set to `false` to forward these responses straight to the client again. The attempt count and backoff are not separate knobs — they reuse `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS` / `_BASE_MS` / `_MAX_MS` above, since both paths are "re-issue the same request once because the upstream is transiently unwell". The retry replays the same request body (Anthropic's Messages API has no idempotency key); this matches what the official Anthropic SDKs do on 5xx by default and what the proxy already does on 529 and on account failover. To keep the bench + failover but skip the in-place replay, leave this `true` and set `CCFLARE_OVERLOAD_RETRY_ENABLED=false` instead; `false` here disables the whole 5xx path (no retry, no bench, no failover). A response carrying `x-should-retry: false` skips the retry and goes straight to the bench; synthetic keepalive/auto-refresh probes are not retried or benched at all. Every send remains subject to the fork physical-attempt budget and authorized account/model plan; hosted operations cannot replay after dispatch | `false` | `CCFLARE_SERVER_ERROR_RETRY_ENABLED=true` |
| `CCFLARE_SERVER_ERROR_COOLDOWN_MS` | Bench applied after a 500/502/503/504 outlives its in-place retry. A shorter upstream `Retry-After` is honored literally; a longer one is clamped to this value. Longer than the 529 cooldown on purpose: an overload clears in seconds, whereas the 500s this exists for lasted minutes at a time on one organization while sibling accounts stayed healthy. Never ramps and never touches `consecutive_rate_limits` | `60000` (60s) | `CCFLARE_SERVER_ERROR_COOLDOWN_MS=30000` |
| `CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED` | Opt-in alias of the client-facing Messages response `model` back to the requested model. Disabled by default to preserve physical served-model identity. Provider processing, pricing, usage accounting and logs continue using the upstream model even when enabled; non-streaming bodies over 256 KiB pass through without aliasing | `false` | `CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED=true` |
