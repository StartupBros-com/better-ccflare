...
| Tool calls announced to the client that do not pair exactly with a completed call — one of several parallel calls left open at the terminal, a completion without a matching start, a reused or absent output index | Never capture. A lineage covering only the completed calls would be an exact-looking subset of the turn upstream actually produced |
