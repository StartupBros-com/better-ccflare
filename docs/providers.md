...
- **Codex** - OpenAI's ChatGPT/Codex Responses API, a bespoke `BaseProvider` (not OpenAI-chat-compatible):
  ...
  - Usage is polled for free from `chatgpt.com/backend-api/wham/usage` — the endpoint the Codex CLI itself polls — every `USAGE_POLL_INTERVAL_MS`; the 5-hour bar is shown only when the plan reports that window (Plus since 2026-08-25; Pro is weekly-only). See the CodexProvider section below
...
8. **Usage Polling (free)**: `supportsUsageTracking` is `true`. The shared `UsageCache` poller GETs `https://chatgpt.com/backend-api/wham/usage` every `USAGE_POLL_INTERVAL_MS` (default 90 s) with `Authorization: Bearer <token>` and `ChatGPT-Account-Id` (the `chatgpt_account_id` claim of the token, read by `extractChatgptAccountId`). This is the same unofficial endpoint the Codex CLI and Codex Desktop poll for their own `/status` display, it consumes no quota, and it is the Codex counterpart of Anthropic's `/api/oauth/usage`. `parseCodexUsagePayload` (`packages/providers/src/providers/codex/usage-endpoint.ts`) slots `rate_limit.primary_window` / `secondary_window` into `five_hour` / `seven_day` by `limit_window_seconds` — never by position — and omits a window the backend did not report: OpenAI removed the 5-hour window on 2026-07-12 and restored it for Plus on 2026-08-25, Pro stays weekly-only, so a Pro payload has no `five_hour` and the dashboard hides that bar instead of drawing a fabricated 0%. Polled windows go through the fork's generation-guarded canonical snapshot callback into `usage_snapshots`, alert evaluation, and the Window Value ledger; polled windows are deliberately not written to `accounts.rate_limit_reset` — the auto-refresh scheduler's `rate_limit_reset <= now` gate and the session-expiry checks need the elapsed value to survive until they act on it, so only real traffic (`response-processor.ts`) and the manual refresh button write that column. Accounts with a custom (non-`chatgpt.com`) endpoint are not polled. The `x-codex-*` headers on real responses still update the cache between polls.
9. **On-Demand Usage Probe (the dashboard refresh button)**: `POST /api/accounts/:id/refresh-usage` reads the free usage endpoint above first (`apps/server/src/codex-usage-refresher.ts`). A 429 from the free endpoint stops the refresh without a quota-consuming fallback. Other empty/error responses, or a custom endpoint without introspection, fall back to one deliberately minimal `/responses` request (a single `.`, `instructions: "ping"`, `stream: true`, `store: false`), cancelling the body as soon as the headers are read (`packages/providers/src/providers/codex/on-demand-fetch.ts`). Three choices in that request are non-obvious and each fixed a real failure:
