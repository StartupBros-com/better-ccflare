...
3. **Default Model Mapping**: All four Claude families (opus, sonnet, haiku, fable) map to `grok-4.7` by default (`XAI_MODEL_MAPPINGS`, compiled at build time). This map is registered as a live-editable "provider model default" factory (`registerProviderModelDefaultFactory("xai", ...)`), so it can be overridden globally via the config-driven override layer — gated behind `CCFLARE_MODEL_DEFAULTS_PROVIDERS` (see `docs/configuration.md`). Note that gate covers only the operator-override editing surface; it does not gate the derived/factory layers below. `beforeConvert()` injects the resolved mapping onto the account only when the account has no explicit `model_mappings` — an account's own pin is never cleared or modified by any of the layers below it, so removing a pin post-deploy falls through to whatever the current derived (or factory) default is at that moment, not to the value that was compiled in when the pin was first set. That fall-through happens only when the request is converted. A route profile's `expectedPhysicalModel` guard runs earlier, at account selection, against the account's own `model_mappings`. So remove a pinned account's mapping only after dropping (or updating) any `expectedPhysicalModel` on profiles that name that account, or those profiles fail closed; see `docs/solutions/integration-issues/route-profile-expected-physical-model-checked-before-provider-defaults.md`.
   - **Catalog-derived override (`packages/providers/src/providers/xai/catalog-defaults.ts`)**: on every models.dev catalog load (see item 7 below), `deriveXaiCatalogDefaults()` picks the newest "bare" release id matching `/^grok-\d+(\.\d+)?$/` (e.g. `grok-5`, not `grok-5-fast-beta`) by the catalog's `release_date` field — a plain string comparison, deliberately not a parsed version-number comparison, so a decoy id that merely *looks* numerically larger (`grok-4.20`) can't outrank a genuinely newer release. Entries with no `release_date` or explicitly marked `tool_call: false` are excluded; `tool_call` left unset is not evidence of `false` and does not exclude an entry. The pick is monotonic — a candidate older than the already-accepted one is ignored outright, so a transient or stale catalog load can never roll xai's default backward — and is published via `setDerivedProviderWideModelDefaults("xai", ...)`, which sits below an account's own derived listing (there is none for xai) but above the compiled `XAI_MODEL_MAPPINGS` factory default in `resolveProviderModelDefault`'s precedence chain. A change logs once at `warn`, naming the old and new model. This derivation is purely reactive to an actual catalog load happening somewhere in the process (dashboard model listing, cost estimation, etc.) — there is deliberately no eager warmup at import time, so a process that never triggers a catalog load keeps the compiled default until one does.
...
6. **Context window advertisement**: Grok 4.5, 4.6, and 4.7 have a 500,000-token xAI window in the bundled fallback table (`packages/core/src/xai.ts`). `resolveXaiContextWindow()` is catalog-first, though: it consults the most recently loaded models.dev "xai" section (via the synchronous `getCatalogModelSummaries("xai")`, exact id then longest-id-prefix match) before ever falling back to the bundled table, and rejects a catalog `limit.context` value outside a [8,000, 10,000,000] sanity bound as a data error rather than trusting it verbatim. This means a new xAI release's window is picked up from the catalog with no code change, as long as models.dev publishes it under the model's own id or an id-prefixed variant. For official `api.x.ai` accounts, the OpenAI→Anthropic stream transform attaches Codex-shaped `message_delta.context_window` (`context_window_size`) with whichever window resolved, so Claude Code autocompacts before that hard stop. Custom xAI-compatible endpoints do not inherit this limit, and original `grok-4` is not treated as 500k.
7. **Long-context tier pricing (`packages/core/src/pricing.ts`)**: when a model's total prompt tokens (input + cache_read + cache_creation) exceed a models.dev-published long-context tier threshold, `estimateCostUSD()` prices the *whole* request at that tier's rates for every token kind, not just the excess — matching xAI's and Anthropic's own published long-context pricing convention (models.dev's schema and docs are silent on whole-request vs. excess-only billing, so this is the documented default rather than an assumption). The resolver prefers a model's explicit `cost.tiers` array over the legacy `cost.context_over_200k` field when both are present (the latter is models.dev's own mechanically-generated mirror of a single `tiers` entry with `size >= 200_000`); a tier that doesn't define a particular cost kind (e.g. no distinct tiered `cache_write` rate) falls back to the model's flat rate for that kind only. A model with no tiers at all prices exactly as before this feature shipped.
