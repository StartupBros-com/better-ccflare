...
- [@anony253](https://github.com/anony253) — fixed cloned request bodies being read via `.json()`, which leaks the body under Bun 1.3.x, by reading as text instead; added regression coverage spying on `Request.prototype.json`/`.text` to catch any future reintroduction of the leaking read, plus hardened the leak-reproduction bench script's mode-argument validation ([#452](https://github.com/tombii/better-ccflare/pull/452))
