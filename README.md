...
- **🤖 Multi-Provider Support** - Claude OAuth, Claude API console, Vertex AI, AWS Bedrock, NanoGPT, z.ai, Minimax, DeepSeek, OpenRouter, Kilo, Codex (OpenAI OAuth), xAI/Grok, Meta Model API (`meta`; legacy `muse-spark` HTTP ingress compatibility), Anthropic-compatible, and OpenAI-compatible providers
...
- **Extended Provider Support** - AWS Bedrock, NanoGPT (with dynamic pricing), Minimax, DeepSeek, OpenRouter, Kilo, Codex (OpenAI OAuth), xAI/Grok, Meta Model API (`meta`; legacy `muse-spark` HTTP ingress compatibility), Anthropic-compatible, and OpenAI-compatible providers
...
- Caller-supplied `previous_response_id` is not trusted as routing or continuation authority; the fork owns continuation state within the authenticated account/model/session lane. See the Codex continuation sections below.
- Freeform custom tools such as Codex's `apply_patch` are bridged through a string input schema on routes translated to Anthropic. Custom tool grammars are described to the model but cannot be enforced by the Anthropic API. Native hosted tools require the fork's reviewed capability route.
...
- Outbound forward proxy support (`BETTER_CCFLARE_OUTBOUND_PROXY`) for enterprise TLS-intercepting egress gateways, with loopback destinations always exempt
...
- **z.ai** - API key based provider with pay-as-you-go model
- **Minimax** - API key based provider with pay-as-you-go model and Token Plan usage polling (utilization tracking for `general` model-class quota)
- **DeepSeek** - Native Anthropic-compatible API at `https://api.deepseek.com`, using API-key authentication. Claude request model IDs pass through for DeepSeek's server-side family remapping; `deepseek-v4-flash` is a response usage/cost-attribution fallback when DeepSeek omits the response model; it does not rewrite outbound request model.
...
- **Meta Model API (`meta`; legacy `muse-spark` HTTP ingress compatibility)** - Meta's Muse Spark models (1M-token context) over the Anthropic Messages API at `https://api.meta.ai`, with bearer-token auth, automatic model mapping to `muse-spark-1.2`, and request normalization for Meta's strict field validation
...
### 🔁 Codex Messages-Lane Continuation (Opt-In, Off by Default)
Lets the Codex provider retain server-owned Responses continuation (`previous_response_id`) while serving Anthropic `/v1/messages` clients, instead of replaying the full translated conversation history on every turn.
- **Off by default** — set `CCFLARE_CODEX_MESSAGES_CONTINUATION=1` to enable. Unset, behavior is unchanged from full-history replay.
- **Optional model allowlist** — `CCFLARE_CODEX_MESSAGES_CONTINUATION_MODELS` restricts the bridge to an exact, comma-separated list of resolved model names; unset enables all models (when the switch above is on), an empty string enables none.
- **Gated on real continuity evidence** — only admitted when the request carries an authenticated API key, a selected account, and a nonempty prompt cache key; state is partitioned by account, model, authenticated caller, session, and protocol, so a Messages-lane checkpoint can never leak into native `/v1/responses` traffic (or vice versa) even for an identical account/model/caller/session.
- **Fails safe, not silent** — an ambiguous, failed, expired, or cancelled turn never creates or reuses a checkpoint; a suspended chain (30-minute TTL) or a rejected `previous_response_id` (upstream `previous_response_not_found` / `invalid_previous_response_id`) triggers a one-time repair that drains the rejected response, retires the checkpoint, and resends full history on the same account and model rather than looping.
- **Never changes routing** — the switch only affects whether Codex replays full history or extends a continuation chain; it never changes model selection, fallback policy, or `store:false` semantics.

See [Environment Variables](docs/configuration.md#additional-environment-variables) for the exact env var reference.
...
- [Account Routing Architecture](docs/routing-architecture.md)
...
- [@Cotch22](https://github.com/Cotch22) - Fixed garbled non-ASCII text in request details modal by decoding base64 bodies as UTF-8 via `TextDecoder` instead of raw `atob()` (PR #246); upstream improvements ([#462](https://github.com/tombii/better-ccflare/pull/462))
...
- [@aleksandrgrn](https://github.com/aleksandrgrn) — fixed the in-place retry for a reset-less Anthropic-shaped 529 being gated on `isRateLimited`, which Zai-like providers never set on overload, so the retry path was silently skipped ([#447](https://github.com/tombii/better-ccflare/pull/447)); fixed Zai's `TOKENS_LIMIT` usage parser dropping the 5-hour window entirely — two `TOKENS_LIMIT` entries (5-hour and weekly) were assigned in loop order so the weekly entry always overwrote the short one — ordered instead by reset time with both windows exposed and utilization/reset kept paired to the same winning window ([#448](https://github.com/tombii/better-ccflare/pull/448)); detected Zai's in-stream `1305` overload event and retried it instead of streaming the overload frame straight to the client ([#449](https://github.com/tombii/better-ccflare/pull/449))
- [@anony253](https://github.com/anony253) — fixed cloned request bodies being read via `.json()`, which leaks the body under Bun 1.3.x, by reading as text instead; added regression coverage spying on `Request.prototype.json`/`.text` to catch any future reintroduction of the leaking read, plus hardened the leak-reproduction bench script's mode-argument validation ([#452](https://github.com/tombii/better-ccflare/pull/452)); upstream improvements ([#460](https://github.com/tombii/better-ccflare/pull/460))
- [@valentinyanakiev](https://github.com/valentinyanakiev) — preserved Codex native-Responses continuation state across turns via authenticated, bounded lane keys, restored stable legacy replay and explicit cache keys, corrected rolling-window reset detection that could discard account affinity prematurely, and added an opt-in passive cache-diagnostics observer (`CCFLARE_CODEX_CACHE_DIAGNOSTICS`) ([#450](https://github.com/tombii/better-ccflare/pull/450)); designed the original Codex Messages-lane continuation — bridging Anthropic `/v1/messages` traffic onto OpenAI's server-owned `previous_response_id` continuation — and rejected-id repair upstream; this fork ported it into its own response-id lifecycle as a strictly opt-in, default-off bridge (`CCFLARE_CODEX_MESSAGES_CONTINUATION`) rather than merging it verbatim, to fit the fork's existing lane-ownership model ([#459](https://github.com/tombii/better-ccflare/pull/459))
- [@aaapl](https://github.com/aaapl) — fixed Anthropic `403 permission_error` responses (an organization disabling OAuth/Claude Code access) matching no failover guard and forwarding straight to the client even with healthy accounts left in the pool, instead of failing over and benching the affected account, with the `org_permission_denied` predicate narrowed to the one known `error_code` so an unrecognized `permission_error` variant isn't benched on an unproven cause ([#435](https://github.com/tombii/better-ccflare/pull/435)); upstream improvements ([#461](https://github.com/tombii/better-ccflare/pull/461))
