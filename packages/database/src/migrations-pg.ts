...
export async function collapseAccountDuplicatesPreservingStatePg(
	...
	for (const grp of groups) {
		...
		const mergedRows = (await adapter.unsafe(
			`SELECT
			   ...
			   (SELECT last_manual_reauth_at FROM accounts
			    WHERE id = ${FRESHEST_CREDENTIAL_ROW_ID}) AS merged_last_manual_reauth_at`,
			[grp.name, grp.provider, grp.ep],
		)) as Array<{
			...
			merged_last_manual_reauth_at: string | null;
		}>;
		const merged = mergedRows[0] ?? {
			...
			merged_last_manual_reauth_at: null,
		};
		...
		await adapter.unsafe(
			...
			`UPDATE accounts SET
			   ...
			   last_manual_reauth_at = $10::BIGINT,
			...
			[
				...
				merged.merged_last_manual_reauth_at, // $10
			],
		);
