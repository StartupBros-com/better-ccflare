...
describe("AccountRepository — usage-threshold pause guards", () => {
	...
	it("ignores threshold writes from a deleted account incarnation", async () => {
		insertAccount(db, "recreated");
		db.run("UPDATE accounts SET created_at = 200 WHERE id = ?", ["recreated"]);
		await repo.pauseForUsageThreshold("recreated", REASON, 100);
		expect(getAccount(db, "recreated")).toEqual({
			paused: 0,
			pause_reason: null,
		});
		await repo.pauseForUsageThreshold("recreated", REASON, 200);
		expect(getAccount(db, "recreated")).toEqual({
			paused: 1,
			pause_reason: REASON,
		});
		await repo.resumeFromUsageThreshold("recreated", REASON, 100);
		expect(getAccount(db, "recreated")).toEqual({
			paused: 1,
			pause_reason: REASON,
		});
		await repo.resumeFromUsageThreshold("recreated", REASON, 200);
		expect(getAccount(db, "recreated")).toEqual({
			paused: 0,
			pause_reason: null,
		});
