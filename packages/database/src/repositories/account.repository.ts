...
export class AccountRepository extends BaseRepository<Account> {
	...
	async markAccountRateLimited(
		...
		let applied = true;
		if (incrementStreak) {
			// Real 429 quota exhaustion: the streak always increments (audit
			// trail of every observed 429/402), and rate_limited_until/reason
			// are clamped forward-only via CASE so a stale/delayed concurrent
			// writer (e.g. a slow retry of an earlier observation) can't land
			// after a fresher, longer cooldown and shorten it back down.
			await this.run(
				`UPDATE accounts
	...
	async pauseForUsageThreshold(
		...
		await this.run(
			`UPDATE accounts SET paused = 1, pause_reason = ? WHERE id = ? AND COALESCE(paused, 0) = 0${expectedCreatedAt === undefined ? "" : " AND created_at = ?"}`,
			[
				reason,
				accountId,
				...(expectedCreatedAt === undefined ? [] : [expectedCreatedAt]),
	...
	async resumeFromUsageThreshold(
		...
		await this.run(
			`UPDATE accounts SET paused = 0, pause_reason = NULL WHERE id = ? AND COALESCE(paused, 0) = 1 AND pause_reason = ?${expectedCreatedAt === undefined ? "" : " AND created_at = ?"}`,
			[
				accountId,
				reason,
				...(expectedCreatedAt === undefined ? [] : [expectedCreatedAt]),
