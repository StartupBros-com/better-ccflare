...
describe("resolveCooldownUntil", () => {
	it("keeps response model aliases off unless explicitly enabled", () => {
		const original = process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED;
		try {
			delete process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED;
			expect(getResponseModelAliasEnabled()).toBe(false);
			process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED = "true";
			expect(getResponseModelAliasEnabled()).toBe(true);
			process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED = "false";
			expect(getResponseModelAliasEnabled()).toBe(false);
		} finally {
			if (original === undefined)
				delete process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED;
			else process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED = original;
		}
	});
...
describe("transient upstream 5xx knobs", () => {
	...
	it("keeps 5xx retries off until explicitly enabled", () => {
