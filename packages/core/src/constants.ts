...
/**
 * Opt-in switch (default off) for the whole transient-5xx path (in-place retry, bench and
...
export function getServerErrorRetryEnabled(): boolean {
	return process.env.CCFLARE_SERVER_ERROR_RETRY_ENABLED === "true";
}

/** Client-facing response aliases are opt-in; preserve physical model identity by default. */
export function getResponseModelAliasEnabled(): boolean {
	return process.env.CCFLARE_RESPONSE_MODEL_ALIAS_ENABLED === "true";
}
