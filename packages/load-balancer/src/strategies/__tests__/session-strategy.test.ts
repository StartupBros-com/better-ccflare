...
describe("SessionStrategy", () => {
	...
	describe("codex upstream window reset ends the session", () => {
		...
		it("stops treating the session as active and re-selects by utilization", async () => {
			const now = Date.now();
			...
			const result = await strategy.select([rolledOver, fresh], meta);

			expect(result[0]).toBe(fresh);
			expect(mockStore.getResetCall("codex-fresh")).toBeDefined();
		});

		it("resets the session when the rolled-over account is the only one", async () => {
			const now = Date.now();
		...
		it("keeps the session while the reported window is still open", async () => {
			const now = Date.now();
			const sessionStart = now - 60 * 60 * 1000;
			const account = makeAccount({
				id: "codex-open",
		...
		it("ignores a reset older than session_start (stale telemetry, not a rollover)", async () => {
			const now = Date.now();
			const sessionStart = now - 60 * 60 * 1000;
			const account = makeAccount({
				id: "codex-stale",
		...
		it("still expires the codex session on the fixed duration", async () => {
			const now = Date.now();
		...
		it("does not apply to zai, whose rate_limit_reset only appears on a 429", async () => {
			const now = Date.now();
			const sessionStart = now - 60 * 60 * 1000;
			const account = makeAccount({
				id: "zai-session",
