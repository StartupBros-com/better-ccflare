		...
		try {
			...
			await db.run(
				...
				[
					...
					// deriveComboRouteClass only tolerates for backward compatibility,
					// and the CLI creation path already writes NULL here.
					null,
					null,
					now + 365 * 24 * 60 * 60 * 1000, // 1 year from now
					now,
 ...
 * minimal real `/responses` request, aborts it after capturing headers, and
 * parses the `x-codex-*` usage values off the response.
 */
