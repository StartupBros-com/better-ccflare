...
describe("createQwenReauthHandler", () => {
	...
	it("should stamp the refresh token issue time and clear the refresh cache", async () => {
		...
		const account = await dbOps.getAdapter().get<{
			...
			"SELECT refresh_token, refresh_token_issued_at FROM accounts WHERE id = ?",
			[accountId],
		);
		expect(account?.refresh_token).toBe("rt");
		expect(account?.refresh_token_issued_at).toBeGreaterThanOrEqual(now);
