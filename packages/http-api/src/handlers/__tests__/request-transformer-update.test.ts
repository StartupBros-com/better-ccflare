...
describe("threshold endpoint eligibility", () => {
	it("rejects a custom Codex endpoint whose usage cannot be polled", async () => {
		let writes = 0;
		const dbOps = {
			getAdapter: () => ({
				get: async () => ({
					name: "custom",
					provider: "codex",
					custom_endpoint: "https://other.example/responses",
				}),
			}),
		...
		const response = await createAccountUsagePauseThresholdsHandler(dbOps)(
			new Request("http://localhost", {
				method: "POST",
				body: JSON.stringify({ fiveHour: 80 }),
			}),
			"custom",
		);
