...
const ACCOUNT_ID = "usage-generation-test";
...
describe("UsageCache polling generations", () => {
	it("does not let a stale generation publish data or invoke its callback", async () => {
	...
	it("does not let stale completion delete the replacement in-flight fetch", async () => {
		const oldFetch = deferred<Response>();
	...
	it("ignores stale failure state and rescheduling", async () => {
		const oldFetch = deferred<Response>();
		const replacementFetch = deferred<Response>();
		let fetchCount = 0;
		globalThis.fetch = (() => {
	...
	it("makes refreshNow generation-safe when stop starts a replacement", async () => {
		const oldFetch = deferred<Response>();
		const replacementFetch = deferred<Response>();
		let fetchCount = 0;
		globalThis.fetch = (() => {
	...
	it("does not let a stale API-key generation publish data", async () => {
		const oldFetch = deferred<Response>();
		const newFetch = deferred<Response>();
		const snapshots: string[] = [];
		let fetchCount = 0;
	...
	it("aborts the stopped registration's provider fetch", async () => {
