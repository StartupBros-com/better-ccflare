...
describe("extractWeeklyResetTime", () => {
	...
	it("returns null for providers without a weekly_all window (zai, xai, unsupported)", () => {
		const emptyUsage = {} as AnyUsageData;
		expect(extractWeeklyResetTime(emptyUsage, "zai")).toBeNull();
		expect(extractWeeklyResetTime(emptyUsage, "xai")).toBeNull();
		expect(extractWeeklyResetTime(emptyUsage, "nanogpt")).toBeNull();
	});
});
