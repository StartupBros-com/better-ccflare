...
describe("model transform does not clone the request body", () => {
	...
	// buffer the body natively and never reclaim it, a ~1x-body-size leak on
...
describe("model transforms preserve the abort signal", () => {
	...
	it("passes the original request through when no rewrite is needed", async () => {
		...
		// The fork's readBodyForTransform always rebuilds the Request (no-clone
		// design, avoiding Bun's native clone-buffer leak — see model-mapping.ts),
		// so identity is never preserved even when the model is unchanged; assert
		// the body content survived unchanged instead.
		expect((await result.clone().json()).model).toBe("same-model");
		controller.abort();
		expect(result.signal.aborted).toBe(true);
	});
});
