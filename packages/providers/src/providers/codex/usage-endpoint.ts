...
const LEGACY_CODEX_USAGE_ENDPOINT = "https://chatgpt.com/api/codex/usage";
let resolvedUsageEndpoint = CODEX_USAGE_ENDPOINT;
...
export async function fetchCodexUsageData(
	...
	const endpoint = options.endpoint ?? resolvedUsageEndpoint;
	...
	try {
		...
		if (response.status === 404 && options.endpoint === undefined) {
			// Keep the fork's symmetric fallback, sharing one timeout and abort
			// signal across both fetches and body consumption.
			try {
			...
			const alternate =
				endpoint === CODEX_USAGE_ENDPOINT
					? LEGACY_CODEX_USAGE_ENDPOINT
					: CODEX_USAGE_ENDPOINT;
			response = await fetch(alternate, {
				method: "GET",
				headers,
				signal: controller.signal,
			});
			if (response.ok) resolvedUsageEndpoint = alternate;
		}
		...
		// Preserve the fork's scalar subscription metadata without treating the
		// separate code-review quota as a routing/usage window.
		if (data && body && typeof body === "object") {
			const details = body as Record<string, unknown>;
			const planType = readCodexPlanType(body);
			if (planType) data.plan_type = planType;
			const credits = details.credits as { balance?: unknown } | undefined;
				...
				const parsed =
					typeof balance === "number"
						...
						: typeof balance === "string" && balance.trim()
							? Number(balance)
							: NaN;
				data.credits_balance = Number.isFinite(parsed) ? parsed : null;
			}
			const review = (
				details.code_review_rate_limit as
					| { primary_window?: CodexUsageWindowPayload }
					| undefined
			)?.primary_window;
			const reviewWindow = review ? toWindow(review, now()) : null;
			if (reviewWindow) {
				data.code_review_used_percent = reviewWindow.utilization;
				data.code_review_resets_at = reviewWindow.resets_at;
			}
		}
		if (!data) {
			log.warn(
