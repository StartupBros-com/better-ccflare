...
test("labels Responses Lite additional tools as native passthrough", async () => {
	const provider = new CodexProvider();
	const requestId = "responses-lite-additional-tools";
	...
	const transformedRequest = await provider.transformRequestBody(request, null);
	const transformedBody = (await transformedRequest.json()) as {
		input: Array<{ type?: string }>;
	};
	expect(transformedBody.input[0]).toMatchObject({
		type: "additional_tools",
	});
