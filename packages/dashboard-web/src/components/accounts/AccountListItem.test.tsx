...
describe("AccountListItem threshold polling eligibility", () => {
	it("hides the threshold control for an account the server cannot poll", () => {
		const html = renderToStaticMarkup(
			<AccountListItem
				account={makeAccount({ provider: "codex", usagePauseSupported: false })}
				{...requiredHandlers}
				onUsageThresholdsChange={() => {}}
			/>,
		);
		expect(html).not.toContain("Set usage pause thresholds");
	});
});
