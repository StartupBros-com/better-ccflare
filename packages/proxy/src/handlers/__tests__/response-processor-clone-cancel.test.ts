...
function makeCtx(opts: {
	...
	// Shared reader used by both provider hooks. The fork's response-processor
	// guards streaming usage extraction with `isStream && parseUsage`
	// (response-processor.ts:232) and non-streaming with `!isStream &&
	// extractUsageInfo` (line 268), so only one of these ever actually runs
	// per test — wiring both to the same implementation keeps the two paths'
	// body-consumption semantics identical for these tests.
	const readUsage = async (response: Response) => {
...
describe("updateAccountMetadata — extractUsageInfo clone lifecycle", () => {
	...
	it("streaming: cancel does not truncate usage extraction (extracted usage is saved)", async () => {
		...
		const { ctx, calls } = makeCtx({
			isStream: true,
			usage,
			// isStream:true routes through provider.parseUsage, not
			// extractUsageInfo (response-processor.ts:232 vs :268); makeCtx
			// wires both hooks to the same reader so this still exercises real
