...
const BACKOFF_ENV_VARS = ["CCFLARE_RATE_LIMIT_MAX_COOLDOWN_MS"] as const;
...
describe("proxyWithAccount — 429 bench honours the provider-parsed reset", () => {
	...
	beforeEach(() => {
		...
		// Preserve the fork's configurable safety ceiling while exercising a
		// provider-confirmed weekly reset beyond the default twelve-hour cap.
		process.env.CCFLARE_RATE_LIMIT_MAX_COOLDOWN_MS = String(
			8 * 24 * 60 * 60 * 1000,
		);
	...
	it("retains the fork safety ceiling for provider-confirmed resets", async () => {
		...
		const ctx = makeProxyContext(() => ({
			isRateLimited: true,
			resetTime: before + FOUR_DAYS_MS,
		}));
		expect(await runProxy(account, ctx)).toBeNull();
		expect(account.rate_limited_until ?? 0).toBeGreaterThanOrEqual(
