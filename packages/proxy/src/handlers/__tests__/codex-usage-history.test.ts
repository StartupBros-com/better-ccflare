		...
		expect(calls[0]?.usage).toEqual([
			{
				windowKey: "five_hour",
				utilization: 12,
				resetsAtMs: Date.parse("2026-07-05T12:00:00Z"),
				scope: "account",
				modelFamily: null,
				active: true,
			},
			{
				windowKey: "seven_day",
				utilization: 63,
				resetsAtMs: Date.parse("2026-07-10T12:00:00Z"),
				scope: "account",
				modelFamily: null,
				active: true,
			},
...
describe("updateAccountMetadata — Codex usage persistence", () => {
	...
	it("persists the parsed windows for a Codex account", async () => {
		...
		const usage = snapshots[0]?.usage ?? [];
		expect(
			usage.find((window) => window.windowKey === "five_hour"),
		).toMatchObject({ utilization: 41, scope: "account", active: true });
		expect(
