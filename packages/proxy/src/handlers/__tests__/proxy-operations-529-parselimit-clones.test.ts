...
describe("issue #354 — 529 path parseRateLimit clones", () => {
	it("the initial 529 parseRateLimit call passes the response directly, no clone", () => {
		...
		expect(source).toMatch(
			/const rlInfo = (provider|attemptPlan)\.parseRateLimit\(response\);/,
		);
		expect(source).not.toMatch(
			/const rlInfo = (provider|attemptPlan)\.parseRateLimit\(response\.clone\(\)\);/,
		);
	});
	...
	it("the in-place retry parseRateLimit call passes the retry response directly, no clone", () => {
		const source = readSource();
		expect(source).toMatch(
			/const retryRlInfo = (provider|attemptPlan)\.parseRateLimit\(retryResponse\);/,
		);
		expect(source).not.toMatch(
			/const retryRlInfo = (provider|attemptPlan)\.parseRateLimit\(retryResponse\.clone\(\)\);/,
		);
	});
});
...
describe("issue #354 — terminal-529 responseForRateLimitCheck clone disposal", () => {
	...
	it("the disposal call sits after processProxyResponse resolves, gated on needsRateLimitCheckClone", () => {
		...
		// 400-char window: the gate sits above an explanatory comment block.
		const gateForDispose = source.slice(disposeIdx - 400, disposeIdx);
		// Fork naming: the gate is `responseForRateLimitCheck !== response`
		// (upstream called it `needsRateLimitCheckClone`).
		expect(gateForDispose).toMatch(
			/if \((needsRateLimitCheckClone|responseForRateLimitCheck !== response)\)/,
		);
	});
});
