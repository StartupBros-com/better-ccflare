...
describe("AutoRefreshScheduler.sendDummyMessage — only OUR OWN refusal is exempt", () => {
	...
	for (const body of [
		JSON.stringify({
			type: "error",
			error: { type: "service_unavailable", code: "route_unavailable" },
		}),
		JSON.stringify({
			error: {
		...
		it(`recognizes the fork local refusal ${body}`, async () => {
			respondWith(503, body);
			const db = makeDb(benched());
			const scheduler = await makeScheduler(db);
			const row = makeAccountRow();
			await scheduler.sendDummyMessage(row);
			expect(scheduler.consecutiveFailures.get(row.id)).toBeUndefined();
			expect(scheduler.uncountedProbeFailures.get(row.id)?.streak).toBe(1);
		});
	}
