...
describe("forwardToClient Anthropic terminal recovery integration", () => {
	...
	it("recovers provider-transformed downstream Anthropic Messages SSE exactly once", async () => {
		const requestHeaders = new Headers({
			"anthropic-version": "2023-06-01",
			"x-better-ccflare-auto-refresh": "true",
			// This fork authenticates internal probes with a process-local secret
			// (internal-probe-auth.ts), so the marker header alone is not enough to
			// classify the request as a probe — it must match ctx.internalProbeSecret.
			"x-better-ccflare-internal-probe-secret": "test-secret",
		});
			...
			const response = await forwardToClient(
				{
					...
					requestHeaders: new Headers({
						"anthropic-version": "2023-06-01",
						"x-better-ccflare-auto-refresh": "true",
						// See note above: this fork requires the probe secret to match.
						"x-better-ccflare-internal-probe-secret": "test-secret",
					}),
					requestBody: bytes("{}"),
		...
		try {
			...
			expect(ends).toEqual([
				expect.objectContaining({
					requestId,
					success: false,
					// Fork divergence: upstream labels this "stream_truncated_mid_content".
					// This fork's semantic-liveness classifier is authoritative for
					// success/error (see response-handler.ts — anthropicOutcome takes
					// precedence), and it labels an SSE close with no terminal event
					// "anthropic_incomplete_eof". Asserted by our own
					// response-handler-worker-protocol.test.ts. streamTerminalState below
					// is upstream's additive contribution and is asserted unchanged.
		...
		try {
			...
			expect(ends).toEqual([
				expect.objectContaining({
					requestId,
					success: false,
					// Fork divergence — see the note on the native-Anthropic case above.
					error: "anthropic_incomplete_eof",
					streamTerminalState: "truncated",
					type: "end",
				}),
	...
	it("preserves success:true and records state=client_cancelled when the client disconnects mid-stream", async () => {
		...
		try {
			...
			expect(cancelEnd).toMatchObject({
				requestId,
				// Fork divergence: upstream records success:true here, reasoning that a
				// client disconnect is not a proxy defect. This fork treats a downstream
				// cancel as a failed request (success:false, error "downstream_cancelled")
				// and that precedence is deliberate and separately covered by
				// response-handler-worker-protocol.test.ts. See the precedence comment in
				// response-handler.ts. streamTerminalState — upstream's actual new
				// contribution — is asserted unchanged.
				success: false,
