...
describe("response alias transport ownership", () => {
	it("does not pull upstream until the client consumes the alias stream", async () => {
		for (const rewrite of [
			...
			const aliased = rewrite(source, "client-model");
			await Bun.sleep(1);
			expect(pulls).toBe(0);
			await new Response(aliased).text();
			expect(pulls).toBe(1);
		}
	});
});
