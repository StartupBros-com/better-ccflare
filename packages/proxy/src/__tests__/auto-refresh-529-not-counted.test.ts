...
function makeDb() {
	...
	return {
		...
		// See auto-refresh-failure-threshold.test.ts for the same pattern.
...
// These three members are `private` on this fork's AutoRefreshScheduler, so an
// intersection with the class type collapses to `never`. Declare the test-only
// surface standalone and cast through `unknown` instead.
type TestableScheduler = {
