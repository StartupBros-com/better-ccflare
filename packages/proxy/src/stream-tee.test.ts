...
describe("teeStream", () => {
	...
	it("propagates cancel(reason) to the upstream reader (fork divergence from upstream drain)", async () => {
		const chunks = [
			textChunk("chunk1"),
			textChunk("chunk2"),
			textChunk("chunk3"),
		...
		// Let any deferred teardown settle.
		await new Promise((resolve) => setTimeout(resolve, 10));

		// FORK DIVERGENCE from upstream 50ec29ba8d (drain-on-cancel): in this
		// fork teeStream wraps the semantic-liveness/terminal-recovery stream
		// chain, not the raw fetch body, so cancel MUST propagate — it runs
		// the inner stream's cancel() handler (records
		// streamTerminalState="client_cancelled") and tears down stalled
		// upstreams during stall-recovery, where a drain would hang forever.
		// See stream-tee.ts's cancel() comment for the full rationale.
