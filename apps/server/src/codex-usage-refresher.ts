...
export interface CodexUsageRefresherDeps {
	...
	resolveEndpoint?(account: Account): string;
	...
	resetSession(accountId: string, expectedCreatedAt?: number): Promise<void>;
...
export function createCodexUsageRefresher(deps: CodexUsageRefresherDeps) {
	const endpointFor = (account: Account) =>
		deps.resolveEndpoint?.(account) ??
		account.custom_endpoint ??
		deps.defaultEndpoint;
	async function currentAccount(account: Account): Promise<Account | null> {
		const current = await deps.getAccount(account.id);
		return current &&
			current.created_at === account.created_at &&
			current.provider === account.provider &&
			(current.custom_endpoint ?? deps.defaultEndpoint) ===
				(account.custom_endpoint ?? deps.defaultEndpoint)
			? current
			: null;
	}
	const discarded = (account: Account): CodexUsageRefreshOutcome => ({
		success: false,
		message: `Account '${account.name}' changed during the refresh — discarded the stale usage snapshot`,
	});
	async function persist(
		...
		if (rolledOver) {
			...
			try {
				await deps.resetSession(account.id, account.created_at);
			} catch (error) {
				deps.log.warn(
		...
		try {
			await deps.updateRateLimitReset(account.id, earliest, account.created_at);
		} catch (error) {
			deps.log.warn(
	...
	return async (accountId: string): Promise<CodexUsageRefreshOutcome> => {
		...
		if (!(await currentAccount(account))) return discarded(account);
		const endpoint = endpointFor(account);

		// 1. Free path.
		if (deps.usageEndpointAvailable(endpoint)) {
			let free: { data: UsageData | null; status: number };
			try {
				free = await deps.fetchFromUsageEndpoint(accessToken);
			} catch (error) {
				deps.log.warn(
					`Codex usage endpoint failed for ${account.name}: ${error}`,
				);
				free = { data: null, status: 0 };
			}
			const current = await currentAccount(account);
			if (!current) return discarded(account);
			if (free.data) {
				...
				return {
					success: true,
					message: `Usage refreshed for '${account.name}'${(free.data.five_hour?.utilization ?? 0) >= 100 || (free.data.seven_day?.utilization ?? 0) >= 100 || (current.rate_limited_until ?? 0) > Date.now() ? " — account is rate limited" : ""} (5h: ${fiveHour}, 7d: ${sevenDay}).`,
				};
			}
			if (free.status === 429) {
		...
		if (probeReset != null) {
			try {
				await deps.updateRateLimitReset(
					account.id,
					probeReset,
					account.created_at,
				);
