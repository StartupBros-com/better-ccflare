...
describe("Codex manual refresh generation safety", () => {
	for (const change of ["deleted", "recreated", "endpoint"] as const) {
		it(`discards an in-flight result after the account is ${change}`, async () => {
			let current: Account | null = makeAccount({ created_at: 1 });
			const { deps, calls } = makeDeps({
				...
				fetchFromUsageEndpoint: async () => {
					current =
						change === "deleted"
							? null
							: makeAccount({
									created_at: change === "recreated" ? 2 : 1,
									custom_endpoint:
										change === "endpoint"
					...
					return {
						data: {
							five_hour: {
								utilization: 20,
								resets_at: new Date(Date.now() + 100000).toISOString(),
							},
						},
						status: 200,
					};
			...
			expect((await createCodexUsageRefresher(deps)("acc-1")).success).toBe(
				false,
			);
			expect(calls.cacheSet).toEqual([]);
	...
	it("falls back when the free endpoint throws", async () => {
		const { deps, calls } = makeDeps({
			fetchFromUsageEndpoint: async () => {
				throw new Error("offline");
			},
		});
		expect((await createCodexUsageRefresher(deps)("acc-1")).success).toBe(true);
		expect(calls.probe).toHaveLength(1);
	});
	it("reports existing response cooldown even when introspection utilization is low", async () => {
		const { deps } = makeDeps({
			getAccount: async () =>
				makeAccount({ rate_limited_until: Date.now() + 100000 }),
		});
		expect((await createCodexUsageRefresher(deps)("acc-1")).message).toContain(
			"rate limited",
		);
	});
});
